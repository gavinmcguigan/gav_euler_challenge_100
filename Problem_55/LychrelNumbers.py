...
def is_it_a_lychrel_number(num: str, attempt: int = 1):
    sum_str = str(int(num) + int(num[::-1]))
    first_part = sum_str[:len(sum_str) // 2]
    second_part = sum_str[-1:(len(sum_str) // 2):-1] if len(sum_str) % 2 else sum_str[-1:(len(sum_str)//2-1):-1]
    ...
    if first_part == second_part:
        ...
        return is_it_a_lychrel_number(sum_str, attempt)


def main():
    total = 0
    for n in range(1, 10000):
        total += is_it_a_lychrel_number(str(n))
